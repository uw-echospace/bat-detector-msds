...
def run_pipeline_with_df(cfg):
    if cfg["recover_folder"] and cfg["sd_unit"]:
        ...
        print(f"Ref Audio Filenames: {(pd.to_datetime(ref_audio_files, '%Y%m%d_%H%M%S', exact=False)).strftime('%Y%m%d_%H%M%S.WAV')}")
        print(f"Good Audio Filenames: {(pd.to_datetime(good_audio_files, '%Y%m%d_%H%M%S', exact=False)).strftime('%Y%m%d_%H%M%S.WAV')}")
